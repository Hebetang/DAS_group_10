---
title: "A Generalized Linear Regression Model Analysis On IMDB rating of film"
...
According to the dataset, we need to transform the character type values into factor type values. Finally, we set the level of the response variable to make the graph of the data visualization more understandable.
...
```
\
**Table \ref{tab:film}** below displays the first five rows of the data after processing. The last column "greater_than_7" was added to each row to present whether the rating of the film is greater than 7.
```{r, eval = TRUE}
...
grid.arrange(p1,p2,p3,p4, 
             ...
             ncol = 2,nrow=2)
```
• **Figure(a)**: There are **zero** potential outliers. The year has little effect on the films' rating, the left and right bars' distributions look the same, and the median of the year distribution of the different rated films is basically in the same level range.

• **Figure(b)**：There are **many potential outliers**. As we can see from the graph, the length of films with high ratings is more densely distributed, while the length of films with poor ratings is spread out.

• **Figure(c)**: There are **eight** potential outlier. In terms of budget, the popular films have slightly higher budgets, but the difference is not too big.

• **Figure(d)**: There are **all potential outliers**. The distributions appear to be similar, but the distribution on the left has more extreme values and requires further discussion.
\

The histogram(**Figure \ref{fig:his}**) shows how many IMDB rates are less than 7 and more than 7 in different movie genres, respectively.
...
We can see that the ratings of different genres of films vary widely, especially Action, Short films, Comedy, Drama, and Romance. short films and comedies are more likely to be loved by audiences in film ratings, while Action, Drama, and Romance are less likely to receive high ratings. This is a rough indication that the genre of film does have an impact on the film scores.
...
•  The point estimate of odds for variable Year is estimated to be positive, which means that, all else being equal, films released later are more likely to receive a higher rating. Considering its high p-value in the parametric test, YEAR has a high probability of being a non-significant variable. 
...
Based on the initial modeling results, the YEAR variable looks more likely to be a non-significant variable, so this variable will be removed and the model will be reconstructed using the remaining variables, resulting in the following.
...
After adjustment and re-modeling, we found that most of the variables were significant, but some variables in the genre were still non-significant. Predicting the results of the model allows us to calculate the probability of an IMDB score greater than 7. 
...
```{r oddsPlot,echo=TRUE,eval = TRUE, fig.align = "center",warning = FALSE,  out.width = '80%',fig.cap = "\\label{fig:Odds} Odds of IMDB rate greater than 7"}
...
```{r, echo = FALSE, fig.align = "center", warning = FALSE, out.width = '80%', fig.cap = "\\label{fig:prob} predicted probabilities of greater than 7"}
plot_model(model1,  type = "pred")
```
**Figure 5** above shows that shorter films are more likely to be predicted as a highly rated film, this may be due to some correlation between LENGTH and SHORT in the film genres, as we can see by doing visualizations before, most of the short films received positive reviews.

**Figure 6**: As with the results of the previous exploratory data analysis, the higher the budget, the more likely it is that the film will be well received in the future.

**Figure 7**: The votes, while also contributing to the film's rating, is less effective in terms of predicting results, due to the graph that the more votes there are, the wider the predicted range.

**Figure 8** above shows that Documentaries are predicted to have a high probability to be rated greater than 7. Comedy and short films are predicted to have a moderate probability to be rated above 7. The probability of other films receiving a rating above 7 is low.
